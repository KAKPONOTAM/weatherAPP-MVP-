...
class HourlyWeatherTableViewCell: UITableViewCell {
    var hourlyWeatherPresenter: HourlyWeatherTableViewPresenterDelegate?
...
extension HourlyWeatherTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return hourlyWeatherPresenter?.weeklyWeatherData?.hourly?.count ?? 0
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        guard let weeklyWeatherData = hourlyWeatherPresenter?.weeklyWeatherData else { return UICollectionViewCell() }
        
        let presenter = HourlyWeatherCollectionViewPresenter(cell: cell)
        cell.presenter = presenter
        presenter.configure(with: weeklyWeatherData, indexPath: indexPath)
        return cell
    }
...
extension HourlyWeatherTableViewCell: HourlyWeatherTableViewCellDelegate {
