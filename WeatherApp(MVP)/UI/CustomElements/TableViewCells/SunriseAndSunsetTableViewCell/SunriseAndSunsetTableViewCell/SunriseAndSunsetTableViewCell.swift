...
class SunriseAndSunsetTableViewCell: UITableViewCell {
    var presenter: SunriseAndSunsetPresenterDelegate?
...
extension SunriseAndSunsetTableViewCell: SunriseAndSunsetCellDelegate {
    func configure(with currentWeatherData: CurrentWeatherInfo) {
        guard let sunriseUnixTime = currentWeatherData.sys?.sunrise,
              let sunsetUnixTime = currentWeatherData.sys?.sunset else { return }
